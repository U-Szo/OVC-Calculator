...
class Ui_SplashScreen(object):
    def setupUi(self, SplashScreen):
        ...
        self.loading_main_frame.setStyleSheet(
                                             "QFrame {    \n"
        ...
        self.loading_progress_label.setStyleSheet(
                                                 "font: 11pt \"Alice\";\n"
                                                 "color: rgb(196, 196, 196);"
                                                 )
...
class SplashScreen(QtWidgets.QMainWindow):
    def __init__(self, window_to_show):
        ...
        self.window_to_show = window_to_show
        
        self.splash_screen_loading_timer = QtCore.QTimer()
        self.splash_screen_loading_timer.timeout.connect(self.loading_progress)
        self.splash_screen_loading_timer.start(12)
        self.show()

    def loading_progress(self):

        self.ui.progress_bar.setValue(self.loading_counter)
        if self.loading_counter > 100:
            self.splash_screen_loading_timer.stop()
            self.window_to_show.show()
            self.window_to_show.ui.set_stylesheets_poststart()

            self.close()

        if self.loading_counter >90:
            self.ui.loading_progress_label.setText("<strong>READY</strong>")

        if self.loading_counter >80 and self.loading_counter <90:
            self.ui.loading_progress_label.setText("<strong>DOING</strong> EVEN MORE STUFF")

        if self.loading_counter >70 and self.loading_counter <80:
            self.ui.loading_progress_label.setText("<strong>DOING</strong> STUFF")

        if self.loading_counter >45 and self.loading_counter <70:
            self.ui.loading_progress_label.setText("<strong>PREPARING</strong> LAUNCH")

        if self.loading_counter >30 and self.loading_counter <45:
            self.ui.loading_progress_label.setText("<strong>READING</strong> FILES")

        if self.loading_counter >20 and self.loading_counter <30:
            self.ui.loading_progress_label.setText("<strong>PARSING</strong> DATA")

        if self.loading_counter >10 and self.loading_counter <20:
            self.ui.loading_progress_label.setText("<strong>LOADING</strong> UI")
